...
```{r Geweke3, echo=FALSE}
knitr::kable(cbind(t(round(geweke[19:27], 4))), align = "l")
```

```{r Geweke4, echo=FALSE}
knitr::kable(cbind(t(round(geweke[28:35], 4))), align = "l")
```
...
```{r ESS gamma, echo=FALSE}
# GAMMA

ess.gamma <-  effectiveSize(jags_post.mcmc)[19:35]

essgammaord <- c(ess.gamma[1],ess.gamma[10:17], ess.gamma[2:9])
colessgammaord <- c(colnames(ess.gamma)[1], colnames(ess.gamma)[10:17], colnames(ess.gamma)[2:9])

essgamma1 <- rbind(colessgammaord[1:9],round(essgammaord[1:9]))
essgamma2 <- rbind(colessgammaord[10:17], round(essgammaord[10:17]))

knitr::kable(essgamma1, align = "l")
knitr::kable(essgamma2, align = "l")
```
...
Furthermore, we can say that all of them have a positive impact on the posterior probabilities of being classified in the **Income** class ">50K", according to the data coming from the Adult Census of 1994.

The results obtained from the **Multivariate gaussian copula model** 
