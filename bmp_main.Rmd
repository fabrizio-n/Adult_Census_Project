...
where $\Phi(z)$ is the cdf of the standard normal distribution. The last line holds because because the diagonal entries of $\Psi$ are all equal to 1, and so the marginal distribution of each $Z_{i,j}$ is a standard normal distribution with cdf $\Psi(z)$.

<br>

We have shown that $F_j(y) = \Phi(g_j^{-1} (y))$, indicating that the marginal distributions of the $Y_j$'s are fully determined by the $g_j$'s and do not depend on the matrix $\Psi$. A model having separate parameters for the univariate marginal distributions, $g_1,...,g_p$, and the multivariate dependencies, $\Psi$, is called a **copula model**, referring to the method of "coupling" a model for multivariate dependence to a model for the marginal distributions of the data.
...
$$R(Y) = \{ Z : z_{i_1, j} < z_{i_2, j} \quad if \quad y_{i_1, j} < y_{i_2, j} \}$$
...
$$\begin{aligned} p(\textbf{Z}|\Sigma) &= \prod_{i=1}^{n} (2 \pi )^{- \frac{p}{2} } |\Sigma|^{- \frac{1}{2}} \exp{ \left\{ -\frac{1}{2} \textbf{z}_i \Sigma^{-1} \textbf{z}_i \right\} } \\ &= (2 \pi)^{-\frac{np}{2}} |\Sigma|^{-\frac{n}{2}} \exp{ \left\{ - \frac{ tr(\textbf{Z}^T \textbf{Z} \Sigma^{-1} )}{2} \right\} } \end{aligned}$$

So the full conditional is given by

$$\begin{aligned} p(\Sigma | \textbf{Z}, \textbf{Z} \in R(\textbf{Y})) = p(\Sigma | \textbf{Z}) &\propto p(\Sigma) \times p(\textbf{Z}| \Sigma) \\ &\propto |\Sigma|^{- \frac{\nu_0 + p +1}{2}} \exp{ \left\{ - \frac{tr(\textbf{S}_0 \Sigma ^{-1} )}{2} \right\}} \times |\Sigma|^{- \frac{n}{2}} \exp{ \left\{ - \frac{ tr(\textbf{Z}^T \textbf{Z} \Sigma^{-1} )}{2} \right\} } \\ &= |\Sigma|^{- \frac{[\nu_0 + n] + p + 1}{2}} \exp{ \left\{ - \frac{tr([\textbf{S}_0 + \textbf{Z}^T \textbf{Z}]  \Sigma ^{-1} )}{2} \right\}}    \end{aligned}$$

which is proportional to an $\mathit{inverse-Wishart} \; (\nu_0 + n, [\textbf{S}_0 + \textbf{Z}^T \textbf{Z}])$ density.
